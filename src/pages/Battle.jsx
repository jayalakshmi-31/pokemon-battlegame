...
function BattlePage() {
  ...
  const startBattle = () => {
    ...
    setTimeout(() => {
      ...
      setBattleLog(log);
      const battleWinner = userWins > computerWins ? "You win!" : computerWins > userWins ? "Computer wins!" : "It's a tie!";
  ...
  }, [battleStage, userPokemonList, fetchComputerPokemon]);

  console.log("Computer Pokémon list:", computerPokemon);
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      {battleStage === "setup" && (
        <>
          <button className="btn btn-primary mb-4" onClick={startBattle}>Start Battle</button>
          <div className="grid grid-cols-2 gap-6 w-full max-w-4xl">
            <div>
              <h2 className="text-xl font-bold">Your Pokémons</h2>
              {userPokemonList?.map((poke) => (
                <div key={poke.id} className="flex items-center gap-4 border p-2">
                  <img src={poke.sprites.front_default} alt={poke.name} className="w-16 h-16" />
                  <span>{poke.name} (Atk: {poke.stats[1].base_stat}, Def: {poke.stats[2].base_stat})</span>
                </div>
              ))}
            </div>
            <div>
              ...
              {computerPokemon.map((poke) => (
                <div key={poke.id} className="flex items-center gap-4 border p-2">
                  ...
                  <span>{poke.name} (Atk: {poke.attack}, Def: {poke.defense})</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {battleStage === "battle" && <h2 className="text-2xl font-semibold mt-40">The battle is happening now...</h2>}
      {battleStage === "result" && (
        <div className="card text-center">
          ...
          <div className="mt-6">
            ...
            <button className="btn btn-primary mt-4" onClick={resetGame}>Play Again</button>
          </div>
        </div>
      )}
    </div>
